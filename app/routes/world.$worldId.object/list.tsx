...
import { ObjectDoc } from "@/world/objects";
import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
...
export default function SceneScrollList({ worldId, objects }: { worldId: WorldId, objects: ObjectDoc[] }) {
  return (
      ...
      <ScrollArea className="h-full">
        <div className="flex flex-col gap-2 p-4 pt-0">
          {objects.map((item) => (
            <Link
              key={item._id}
              to={`/world/${worldId}/object/${item._id}`}
              className={cn(
